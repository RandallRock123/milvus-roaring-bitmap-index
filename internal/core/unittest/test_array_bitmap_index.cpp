...
class LargePermissionBitmapTest : public testing::Test {
protected:
    void SetUp() override {
        index_ = std::make_unique<BitmapIndex<int64_t>>();
        values_.resize(LARGE_BITMAP_THRESHOLD);
        std::iota(values_.begin(), values_.end(), 0);
    }

    std::unique_ptr<BitmapIndex<int64_t>> index_;
    std::vector<int64_t> values_;
};

TEST_F(LargePermissionBitmapTest, TestBuildMode) {
    index_->Build(values_.data(), values_.size());
    EXPECT_EQ(index_->GetBuildMode(), BitmapIndexBuildMode::ROARING);
}

TEST_F(LargePermissionBitmapTest, TestRangeQuery) {
    index_->Build(values_.data(), values_.size());
    
    auto start = std::chrono::high_resolution_clock::now();
    auto result = index_->Range(LARGE_BITMAP_THRESHOLD/2, OpType::GreaterEqual);
    auto end = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
    
    EXPECT_TRUE(result.test(LARGE_BITMAP_THRESHOLD - 1));
    EXPECT_FALSE(result.test(0));
}

TEST_F(LargePermissionBitmapTest, TestMemoryUsage) {
    size_t before_size = values_.size() * sizeof(int64_t);
    index_->Build(values_.data(), values_.size());
    
    auto roaring_bitmap = index_->GetRawBitmap(0);
    size_t after_size = roaring_bitmap.getSizeInBytes();
    
    EXPECT_LE(after_size * 3, before_size);
}

TEST_F(LargePermissionBitmapTest, TestRangeQueryWithBinning) {
    index_->Build(values_.data(), values_.size());
    
    int64_t lower = LARGE_BITMAP_THRESHOLD / 4;
    int64_t upper = LARGE_BITMAP_THRESHOLD / 2;
    
    auto result = index_->Range(lower, true, upper, true);
    
    EXPECT_TRUE(result.test(lower));
    EXPECT_TRUE(result.test(upper));
    EXPECT_FALSE(result.test(lower - 1));
    EXPECT_FALSE(result.test(upper + 1));
}
